...
func Registration(w http.ResponseWriter, r *http.Request) {
    ...
    _,err = database.DB.Exec("INSERT INTO users (username, password, email,name) VALUES (?,?,?,?)", creds.Username,hashedPassword, creds.Email,creds.Name)
    if err != nil {
...
func Login(w http.ResponseWriter, r *http.Request){
    ...
    var user user.User
    err = database.DB.QueryRow("SELECT username, password FROM users WHERE username= (?)", creds.Username).Scan(&user.Username, &user.Password)
    if err != nil {
